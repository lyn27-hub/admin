    ...
    public function adminStatistics()
{
    $topFoods = DB::table('food_logs')
        ...
        ->groupBy('foods.nama_makanan')
        ->orderByDesc('total')
        ->limit(5)
        ->get();

    $highCalories = DB::table('food_logs')
        ->join('foods', 'food_logs.food_id', '=', 'foods.id')
        ->select(
            'foods.nama_makanan',
            DB::raw('SUM(food_logs.total_kalori) as total_kalori')
        )
        ->groupBy('foods.nama_makanan')
        ->orderByDesc('total_kalori')
        ->limit(5)
        ->get();
    ...
    $trend = DB::table('food_logs')
        ->select(
            'tanggal_catat',
            DB::raw('SUM(total_kalori) as total_kalori')
        )
        ->groupBy('tanggal_catat')
