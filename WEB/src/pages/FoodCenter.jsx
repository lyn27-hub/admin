...
function FoodModal({ food, onClose, onSaved }) {
  ...
  const handleSubmit = async () => {
    if (!form.nama_makanan || !form.kalori_standar || !form.satuan_standar) {
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div>
            ...
            <input
              type="number"
              value={form.kalori_standar}
              onChange={(e) => setForm({ ...form, kalori_standar: e.target.value })}
              placeholder="cth: 483"
              style={inputStyle}
            />
          </div>
          ...
          <div>
            ...
            <input
              value={form.satuan_standar}
              onChange={(e) => setForm({ ...form, satuan_standar: e.target.value })}
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <p style={{ color: "#9CA3AF", fontSize: 13, margin: "0 0 24px" }}>
          <strong style={{ color: "#F9FAFB" }}>{food.nama_makanan}</strong> akan dihapus permanen dari database.
        </p>
    ...
    <div style={{
      ...
      <div style={{ display: "flex", gap: 14, marginBottom: 20 }}>
        ...
        <StatCard icon="🔥" label="Makanan Tertinggi Kalori"
          value={highCalories[0]?.nama_makanan || "-"} color="#FBBF24" />
        <StatCard icon="📈" label="Makanan Terpopuler"
          value={topFoods[0]?.nama_makanan || "-"} color="#34D399" />
      </div>
              ...
              {filteredFoods.length === 0 ? (
                ...
                filteredFoods.map((food, index) => (
                  <tr key={food.id}>
                    ...
                    <td style={tableCell}>{food.nama_makanan}</td>
                    <td style={tableCell}>{food.kalori_standar}</td>
                    <td style={tableCell}>{food.satuan_standar}</td>
                    <td style={tableCell}>
        ...
        <SectionCard title="Makanan Paling Sering Dikonsumsi">
          {topFoods.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <PieChart>
                <Pie data={topFoods} dataKey="total" nameKey="nama_makanan" outerRadius={90}>
          ...
          {highCalories.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={highCalories}>
                ...
                <XAxis dataKey="nama_makanan" tick={{ fill: "#9CA3AF", fontSize: 11 }} />
